...
public class T01FileTest {
	public static void main(String[] args) throws IOException {
		...
		File file = new File("d:/Others/test.txt");
		System.out.println("파일명 : " + file.getName());
		System.out.println("파일 여부 : " + file.isFile());
		System.out.println("디렉토리(폴더) 여부 : " + file.isDirectory());
		System.out.println("--------------------------------------------");
		
		File file2 = new File("d:/Others");
		...
		System.out.print(file3.getName() + "의 용량 : " + file3.length() + "(bytes)");
		
		//3. new File(String parent, String child)
		File file4 = new File(".\\Others\\test\\..", "test.txt");//상대경로
		...
		/*
	 	...
	 	2.mkdirs() -> 중간의 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치에 디렉토리를 만들어 준다.
	 	
	 	=> 위 두 메소드 모두 만들기를 성공하면 true, 실패하면 flase를 반환함.
		 */
		File file5 = new File("d:/Others/연습용");
		if(file5.mkdir()) {
		...
		File file6 = new File("d:/Others/test/java/src");
		if(file6.mkdirs()) {
